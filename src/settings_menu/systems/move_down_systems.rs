...
pub fn interact_with_move_down_button(
    button_colors: Res<UIMenuButtonColors>,
    mut button_query: Query<
        (&Interaction, &mut BackgroundColor, &mut MoveDownButton),
        With<MoveDownButton>,
    >,
    ...
    for (idx, (interaction, mut background_color, mut move_down_button)) in
        button_query.iter_mut().enumerate()
    {
        match *interaction {
            Interaction::Pressed => {
                if !move_down_button.pressed {
                    move_down_button.pressed = true;
                    pkv.set_string(
                        move_down_button.key.clone(),
                        &move_down_button.value.to_string(),
                    )
                    .expect("failed to save total colors");
                    *background_color = button_color_by_interaction(
                        move_down_button.pressed,
                        &button_colors,
                        &move_down_button.color_type,
                        interaction,
                    )
                    .into();
                }
            ...
            Interaction::None => {
                if pressed_button.0 > -1
                    ...
                    && pressed_button.1 == move_down_button.key
                {
                    if move_down_button.pressed {
                        move_down_button.pressed = false;
                    }
                }
                *background_color = button_color_by_interaction(
                    move_down_button.pressed,
